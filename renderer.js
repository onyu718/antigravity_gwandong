...
async function getFallbackGwandongData() {
  ...
  return [
    ...
    },
    {
        ...
        "lat": 38.4526,
        "lng": 127.608,
        "original_text": "동주(東州) 밤 겨우 새와 북관뎡(北寬亭) 오라 하니,\n삼각산(三角山) 제일봉(第一峰)이 하마면 뵈리로다.\n궁왕(弓王) 대궐(大闕) 터희 오쟉(烏鵲)이 지지괴니,\n천고(千古) 흥망(興亡)을 아는다 모라는다.\n회양(淮陽) 네 일홈이 마초아 가탈시고.\n급장유(汲長孺) 풍채(風采)를 고텨 아니 볼 게이고.",
    ...
    },
    {
        ...
        "lat": 38.6496,
        "lng": 128.0931,
        "original_text": "영즁(營중)이 무사(無事)하고 시절(時節)이 삼월(三월)인 제,\n화천(花川) 시내길이 풍악(楓岳)으로 열려 잇다.\n행장(행장)을 다서치고 석경(石徑)에 막대 디퍼,\n백천동(百川洞) 겨태 두고 만폭동(萬瀑洞) 드러가니,\n은(銀) 무지게 가탄 옷(衣) 용(龍)의 초리 가탄 서리러라.\n들을 제난 우레러니 보니난 눈이로다.",
    ...
    },
    {
        ...
        "lat": 38.652,
        "lng": 128.085,
        "original_text": "금강대(金剛臺) 맨 우등에 선학(仙鶴)이 새끼 치니,\n춘풍(春風) 옥뎍(玉笛) 소리에 첫잠을 깨돗던디,\n호의현상(縞衣玄裳)이 반공(半空)에 소소 뜨니,\n서호(西湖) 녯 주인을 반겨서 넘노난 듯.",
    ...
    },
    {
        ...
        "lat": 38.648,
        "lng": 128.09,
        "original_text": "소향로(小香爐) 대향로(大香爐) 눈 아래 구버보고,\n정양사(正陽寺) 진헐대(眞歇臺) 고텨 올라 안잔마리,\n녀산(廬山) 진면목(眞面目)이 여기야 다 뵈난다.\n어와 조화옹(造化翁)이 헌사토 헌사할샤.\n날거든 뛰디 마나, 셧거든 솟디 마나.",
    ...
    },
    {
        ...
        "lat": 38.655,
        "lng": 128.092,
        "original_text": "개심대(開心臺) 고텨 올라 중향셩(衆香城) 바라보며,\n만이천봉(萬二천봉)을 력력(歷歷)히 헤여 하니,\n봉(峰)마다 맷쳐 잇고 끝마다 서린 긔운,\n맑거든 조티 마나, 조거든 맑디 마나.\n뎌 긔운 흩내여 인걸(人傑)을 만들고쟈.",
    ...
    },
    {
        ...
        "lat": 38.645,
        "lng": 128.08,
        "original_text": "외나무 다리 아래 화룡소(化龍沼) 되어 세라.\n천만(千萬) 구븨 늘근 용(龍)이 구븨구븨 서려 이셔,\n주야(晝夜)의 흘러 내여 창해(滄海)예 니어시니,\n풍운(風雲)을 언제 얻어 삼일우(三일雨)를 디련는다.\n음애(陰崖)예 이온 풀을 다 살와 내여사라.",
    ...
    },
    {
        ...
        "lat": 38.65,
        "lng": 128.1,
        "original_text": "마하연(摩訶衍) 묘길상(妙吉祥) 안문(雁門)재 넘어디나,\n외나무 썩은 다리 불정대(佛頂臺) 오라 하니,\n천심절벽(천심절벽)을 반공(반공)에 세워 두고,\n십이폭포(十二瀑布)를 은하수(銀河수) 한 구븨를 촌촌이 버혀 내여,\n실같이 플텨이셔 베같이 거러시니,\n도경(圖經) 열두 구븨, 내 보기는 여러히라.",
    ...
    },
    {
        ...
        "lat": 37.7513,
        "lng": 128.8761,
        "original_text": "산즁(山中)을 매양 보랴 동해(東海)로 가쟈사라.\n남여(籃輿) 완보(緩步)하여 산영루(山映樓)에 올라 하니,\n영롱(玲瓏) 벽계(碧溪)와 수성(數聲) 제조(啼鳥)난 니별(離別)을 원(怨)하는 듯,\n졍기(旌旗)를 떨티니 오색(오색)이 넘노난 듯,\n고각(鼓角)을 섞어 부니 해운(海雲)이 다 걷는 듯.",
    ...
    },
    {
        ...
        "lat": 38.9328,
        "lng": 127.8931,
        "original_text": "금란굴(金蘭窟) 겨태 두고 총석정(叢石亭) 올라 하니,\n백옥루(白玉樓) 남은 기동 다만 네히 셔 잇고야.\n공수(工倕)의 셩기(聖器)인가 헌원(軒轅)의 침삭(針削)인가.\n구태여 륙면(六面)은 므서스를 형상(形象)턴고.",
    ...
    },
    {
        ...
        "lat": 38.6515,
        "lng": 128.2708,
        "original_text": "삼일포(三일浦)를 찾아가니 단서(丹書)난 완연(宛然)하되,\n이샤선(四仙)은 어디 가고 삼일(일) 머믄 후(後)에 어디 가 또 머믈고.\n선유담(仙遊潭) 영랑호(永郞湖) 거기나 가 잇난디.\n청간정(淸澗亭) 만경대(萬景臺) 몇 고대 안잣던고.",
    ...
    },
    {
        ...
        "lat": 37.7944,
        "lng": 128.8967,
        "original_text": "사양(斜陽) 현산(峴山) 텰튁을 무수히 밟아,\n우개지륜(羽蓋芝輪)이 경포(鏡浦)로 나려가니,\n십리(十里) 빙환(氷紈)을 다려 고텨 다려,\n장송(長松) 울창(鬱蒼)한 속에 슬카장 펴뎌 쓰니,\n물결이 잔잔하여 모래를 혜리로다.\n고주해람(孤舟해람)하여 정자(亭子) 우희 올라가니...",
    ...
    },
    {
        ...
        "lat": 37.4418,
        "lng": 129.1604,
        "original_text": "진주관(眞珠館) 쥭서루(竹西樓) 오십천(五十川) 내린 물이,\n태백산(太白山) 그림자를 동해로 담아 가니,\n차라리 한강(漢江)의 목멱(木覓)에 다히고쟈.\n왕뎡(王程)이 유한(有限)하고 풍경(風景)이 못 슬믜니,\n유회(幽懷)도 하도 할샤, 객수(객수)도 둘 데 없다.",
